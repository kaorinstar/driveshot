...
作業中は `windows` にしています。macOSのビルドは1回あたり約10分かかり、macOS側の作業を始めるまで
得られるものがないためです。その代わり、その間はmacOSでのコンパイルが一度も行われません。手動実行で
**Platforms** に `macos` を選べば、コミットせずに確認できます。両方のプラットフォームを書く段階に
なったら `both` に戻します。`v*` タグはこの設定を参照しません。リリースは常に両方のパッケージを
作ります。
