...
`APP_PLATFORMS` に書いてあります。値は `windows`・`macos`・`both` のいずれかです。Windows向けの
作業中は `windows` にしています。macOS側の作業を始めるまで、macOSのビルドから得られるものがない
ためです。2つのジョブは同時に動き、長いのはWindowsのほうです（両方をビルドした直近の実行では
Windowsが12分16秒、macOSが4分20秒。Windowsはキャッシュが効くと5分5秒です）。そのため、実行時間は
あまり短くなりません。減るのは課金対象の
時間です。このリポジトリは非公開で、macOSの実行時間はLinuxの10倍で課金されます。その実行では、
課金対象80分のうち約50分がmacOSのジョブでした。その代わり、その間はmacOSでのコンパイルが一度も
行われません。手動実行で **Platforms** に `macos` を選べば、コミットせずに確認できます。両方の
プラットフォームを書く段階になったら `both` に戻します。`v*` タグはこの設定を参照しません。
リリースは常に両方のパッケージを作ります。

3つ目の `.github/workflows/report-build-status.yml` は、上記2つのジョブ終了後に呼ばれます。
